...
class TestBlackBoxMetamodelClassification(TestCase):
    ...
    def test_create_gbm_model_by_name(self):
        from uq360.algorithms.blackbox_metamodel import blackbox_metamodel_classification as bb
        X, y = self._generate_mock_data(200, 4, 3)
        m = bb.BlackboxMetamodelClassification(base_model='gbm', meta_model='lr')
        m.fit(X, y)
    ...
    def test_create_gbm_model_from_class(self):
        ...
        m = bb.BlackboxMetamodelClassification(base_model=GradientBoostingClassifier, meta_model=LogisticRegression)
        m.fit(X, y)
        y_pred, y_prob = m.predict(X[0:5, :])
        assert (all(y_pred == [3, 3, 0, 1, 0]))
        assert (all(np.isclose(y_prob, [0.52529918, 0.23764194, 0.93118627, 0.80335416, 0.95338512], rtol=1.e-03)))
    ...
    def test_create_gbm_model_from_instance(self):
        ...
        m = bb.BlackboxMetamodelClassification(base_model=gbm_base, meta_model='lr')
        m.fit(X, y)
        y_pred, y_prob = m.predict(X[0:5, :])
        assert (all(y_pred == [3, 3, 0, 1, 0]))
        assert (all(np.isclose(y_prob, [0.52529918, 0.23764194, 0.93118627, 0.80335416, 0.95338512], rtol=1.e-03)))
        ...
        m = bb.BlackboxMetamodelClassification(base_model=gbm_base, meta_model=lr_meta)
        m.fit(X, y)
        y_pred, y_prob = m.predict(X[0:5, :])
        assert (all(y_pred == [3, 3, 0, 1, 0]))
        assert (all(np.isclose(y_prob, [0.52529918, 0.23764194, 0.93118627, 0.80335416, 0.95338512], rtol=1.e-03)))
